  ...
  for (const file of files) {
    for (const script of extractScripts(file)) {
      ...
      walk(ast, (node) => {
        ...
        const gasFunction =
          findGoogleScriptRunMethod(node);
...
function belongsToGoogleScriptRunChain(
  node
) {
  if (
    !node ||
...
function extractInlineHandlers(
  files
) {
  const handlers = [];
...
function findMissingGasFunctions(
  googleCalls,
  gasFunctions
) {
  const gasNames =
...
function findMissingDomReferences(
  domReferences,
  htmlElements
) {
  const existing =
...
function main() {
  ...
  console.log(
    `📜 Blocs <script>           : ${parsing.total}`
  );

  console.log(
    `✓ Scripts parsés            : ${parsing.parsed}/${parsing.total}`
  );
  ...
  if (!unused.length) {
    ...
    console.log(
      "    une fonction peut être appelée depuis un autre"
    );

    console.log(
      "    fichier HTML, un attribut inline ou du contexte GAS."
    );
  }
  ...
  const missingGas =
    findMissingGasFunctions(
      googleCalls,
      gasFunctions
    );

  printTitle(
    "APPELS GAS SANS FONCTION CORRESPONDANTE"
  );
  ...
  if (!missingGas.length) {
    ...
    for (
      ...
      console.log(
        `    appelé depuis ${call.file}:${call.line}`
      );
    }
  }
    ...
    for (
      const handler of
        uniqueBy(
          inlineHandlers,
          item =>
            `${item.event}:${item.name}:${item.file}:${item.line}`
        )
    ) {
      console.log(
  ...
  const missingDom =
    findMissingDomReferences(
      domReferences,
      htmlElements
    );

  printTitle(
    "RÉFÉRENCES DOM INEXISTANTES"
  );
  ...
  console.log(
    `  Fonctions front                   ${functions.length}`
  );

  console.log(
    `  Fonctions potentiellement mortes ${unused.length}`
  );

  console.log(
    `  Appels google.script.run          ${googleCalls.length}`
  );

  console.log(
    `  Appels GAS inexistants             ${missingGas.length}`
  );

  console.log(
    `  Références DOM inexistantes        ${missingDom.length}`
  );

  console.log(
    `  IDs dupliqués                      ${duplicateIds.length}`
  );
