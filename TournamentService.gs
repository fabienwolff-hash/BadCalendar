...
const TournamentService = {
  read() {
    const rows = this.readRows_();

    const tournaments = this.buildTournaments_(rows).map((tournament) => this.enrich_(tournament));
  ...
  buildTournaments_(rows) {
    const groups = this.groupByTournamentId_(rows);

    return Object.keys(groups).map((tournamentId) =>
      this.buildTournament_(tournamentId, groups[tournamentId])
    );
  },
  ...
  buildTournament_(tournamentId, rows) {
    ...
    const programs = this.buildPrograms_(rows);
  ...
  buildPrograms_(rows) {
    const programs = rows.map((row) => this.buildProgram_(row));
  ...
  buildSites_(row) {
    ...
    if (!site.region && !site.department && !site.city && !site.gymnasium) {
      return [];
    }
  ...
  enrich_(tournament) {
    const startDate = this.getTournamentStartDate_(tournament);

    const endDate = this.getTournamentEndDate_(tournament);

    const registrationStatus = this.computeRegistrationStatus_(
      tournament.registration.openDate,
      tournament.registration.closeDate
    );

    const tournamentStatus = this.computeTournamentStatus_(startDate, endDate);

    const siteCount = this.getUniqueSites_(tournament).length;

    const calendarLocation = siteCount > 1 ? "Voir BadNet" : this.buildDisplayLocation_(tournament);
    ...
    const enrichedTournament = {
      ...
      displayDate: this.buildDisplayDate_({
        startDate,
        endDate,
      }),

      displayLocation: this.buildDisplayLocation_(tournament),

      googleCalendarUrl: CalendarService.buildGoogleCalendarUrl({
      ...
      month: startDate ? startDate.toLocaleString(CONFIG.LOCALE, { month: "long" }) : "",

      monthNumber: startDate ? startDate.getMonth() + 1 : null,
  ...
  getTournamentStartDate_(tournament) {
    const dates = tournament.programs.map((program) => program.startDate).filter(Boolean);
  ...
  getTournamentEndDate_(tournament) {
    const dates = tournament.programs.map((program) => program.endDate).filter(Boolean);
  ...
  buildDisplayLocation_(tournament) {
    const sites = this.getUniqueSites_(tournament);

    if (sites.length === 0) {
      const department = this.getSingleDepartment_(tournament);
      ...
      const region = this.getSingleRegion_(tournament);
  ...
  buildDisplayDate_(eventLike) {
    const start = eventLike.startDate;

    const end = eventLike.endDate;
    ...
    return `${this.formatDateWithoutWeekday_(start)} - ${this.formatDateWithoutWeekday_(end)}`;
  },
  ...
  serialize_(tournament) {
    return {
      ...
      startDate: this.serializeDate_(tournament.startDate),

      endDate: this.serializeDate_(tournament.endDate),

      creationDate: this.serializeDate_(tournament.creationDate),

      registration: {
        ...tournament.registration,

        openDate: this.serializeDate_(tournament.registration.openDate),

        closeDate: this.serializeDate_(tournament.registration.closeDate),
      },

      programs: tournament.programs.map((program) => this.serializeProgram_(program)),
    };
  },
  ...
  serializeProgram_(program) {
    return {
      ...
      startDate: this.serializeDate_(program.startDate),

      endDate: this.serializeDate_(program.endDate),
    };
  },
